...
def test_compute_rmse():
    # Case1:
    nparray1 = np.array([[5, 6], [7, 8], [1, 2], [3, 4]])
    nparray2 = np.array([[8, 9], [7, 6], [4, 3], [0, 0]])
    ...
    np.testing.assert_allclose(compute_rmse(nparray3, nparray4), np.array([4.295366, 4.93455]), rtol=1e-6, atol=0)
...
def test_get_all_pairs():
    ...
    expected = [(1, 2.3), (1, 3), (1, 4), (1, 5), (2.3, 3), (2.3, 4), (2.3, 5), (3, 4), (3, 5), (4, 5)]
...
def test_otsu_threshold_cc(size):
    ...
    print(
        f"thr difference for size={size}: skimage={otsu_skimage}, custom={otsu_custom}, diff={abs(otsu_custom-otsu_skimage)}"
    )
