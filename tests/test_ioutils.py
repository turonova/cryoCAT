...
@pytest.mark.parametrize(
    "content, encoding, expected_encoding",
    [
        ("hello", "utf-8", "utf-8"),
...
def test_cttfind4_read():
    ...
    pd.testing.assert_frame_equal(
        df_test18, ctffind4_read(str(current_dir / "test_data" / "TS_018" / "018_ctffind4.txt"))
    )
...
def test_defocus_load():
    ...
    pd.testing.assert_frame_equal(
        test_df_xml, defocus_load(str(current_dir / "test_data" / "TS_018" / "018.xml"), "warp")
    )
    ...
    pd.testing.assert_frame_equal(
        df_test_star, defocus_load(str(current_dir / "test_data" / "TS_018" / "018_gctf.star"), "gctf")
    )
    ...
    pd.testing.assert_frame_equal(
        df_test18_ctffind4, defocus_load(str(current_dir / "test_data" / "TS_018" / "018_ctffind4.txt"), "ctffind4")
    )

    # Test defocus_load passing a pd dataframe with any extension file
    pd.testing.assert_frame_equal(df_test18_ctffind4, defocus_load(df_test18_ctffind4, "ctffind4"))
    pd.testing.assert_frame_equal(df_test18_ctffind4, defocus_load(df_test18_ctffind4, "warp"))
    pd.testing.assert_frame_equal(df_test18_ctffind4, defocus_load(df_test18_ctffind4, "gctf"))
    # random extension, shouldn't affect it, meaningless
    pd.testing.assert_frame_equal(df_test18_ctffind4, defocus_load(df_test18_ctffind4, "random"))
    ...
    pd.testing.assert_frame_equal(df_numpy, defocus_load(numpy_array, "random"))
...
def create_test_csv_dose(correctedDose=None, removed=None):
    ...
    if removed is None:
        ...
        data = {"CorrectedDose": correctedDose, "Removed": removed}
...
def test_dict_write():
    ...
    # example dictionary
    dictionary1 = {
        "person1": {"name": "Alice", "age": 30, "skills": ["Python", "Data Analysis"], "is_student": False},
        "person2": {"name": "Bob", "age": 25, "skills": ["JavaScript", "React"], "is_student": True},
    }
    try:
...
def test_z_shift_load(csv_file):
    ...
    expected_result = pd.DataFrame({"tomo_id": [1.0, 2.0], "z_shift": [100.0, 200.0]})
    pd.testing.assert_frame_equal(result, expected_result)
...
def test_dimensions_load(csv_file2):
    ...
    pd_input_test1 = pd.DataFrame(
        [[0, 1, 2, 3], [1, 1.1, 2.1, 3.1], [2, 1.2, 2.2, 3.2]], columns=["tomo_id", "x", "y", "z"]
    )
    ...
    angles_mdoc018_test2values = [-52.0064, -50.0066]
...
def test_indices_load(tmp_path):
    ...
    df = pd.DataFrame({"ToBeRemoved": [True, False, True, False], "OtherColumn": [10, 20, 30, 40]})
    ...
    expected = np.array([0, 2])  # Only rows where "Removed=False" and "ToBeRemoved=True"
...
def test_indices_reset(tmp_path):
    ...
    pd.testing.assert_frame_equal(modified_df, expected_df, check_dtype=True, obj="DataFrame after indices_reset")

    # Case: Input file without "Removed" column
    csv_path_no_removed = tmp_path / "test_no_removed.csv"
    df_no_removed = pd.DataFrame({"ToBeRemoved": [True, False, True, False], "OtherColumn": [10, 20, 30, 40]})
    ...
    expected_df_no_removed = pd.DataFrame(
        {"ToBeRemoved": [False, False, False, False], "OtherColumn": [10, 20, 30, 40]}  # Reset to False
    )

    # Assert that the DataFrame matches the expected result
    pd.testing.assert_frame_equal(
        modified_df_no_removed,
        expected_df_no_removed,
        check_dtype=True,
...
def test_get_all_files_matching_pattern(setup_temp_directory):
    ...
    file_names, wildcards = get_all_files_matching_pattern(pattern, numeric_wildcards_only=True, return_wildcards=True)
    ...
    pattern = os.path.join(str(test_dir), "file4_*.doc")
    file_names, wildcards = get_all_files_matching_pattern(pattern, numeric_wildcards_only=False, return_wildcards=True)
    expected_files = [os.path.join(test_dir, "file4_123.doc"), os.path.join(test_dir, "file4_abc.doc")]
...
def test_sort_files_by_idx():
    ...
    with pytest.raises(ValueError, match="idx_list contains invalid indices"):
        assert np.array_equal(
            sort_files_by_idx(["test.txt", "test2.txt"], ["1", "1"]), np.array(["test.txt", "test2.txt"])
        )
    ...
    assert np.array_equal(
        sort_files_by_idx(["test1.txt", "test2.txt"], ["1", "2"], "descending"), np.array(["test2.txt", "test1.txt"])
    )
...
def test_relion_ctffind4_read_direct():
    """Directly invoke the loader; must return the canonical 5-column defocus DataFrame."""
    file_path = Path(__file__).parent / "test_data" / "dc02t01_ctf.star"
    result = ioutils.relion_ctffind4_read(str(file_path))
