...
def project_equidistant(coord):
    ...
    projection_theta_r_local[:,1] = convert_to_radial(projection_xy_local)[0] #np.cos(( np.pi + theta )/2) #np.cos( theta  - np.pi/2) 
...
def plot_polar_nn_distances(coordinates, distances, max_radius = None, marker_size = 3, colormap = 'viridis_r', graph_title = None):
...
def plot_orientational_distribution(coordinates, projection = 'stereo', graph_title=None, theta_bin=73, radius_bin=33, max_radius = None, colormap = 'viridis_r'):
