...
def expand_points(points, normals, distances, tb_distances = 0):
    ...
    if tb_distances == 0: # 0 for nonmove top and bottom
        post_points[moved_points] = post_points[moved_points] + distances*normals[moved_points]
    elif tb_distances != 0: # other value for move top and bottom specific distance
        post_points[moved_points] = post_points[moved_points] + distances*normals[moved_points]
        post_points[tb_points] = post_points[tb_points] + tb_distances*normals[tb_points]

    # removing the distinct points after shifting
    drop_lim = [max(post_points[tb_points][:, 0]), min(post_points[tb_points][:, 0])]
    drop_list = [i for i in range(len(post_points)) if post_points[i, 0] > drop_lim[0] or post_points[i, 0] < drop_lim[1]]
...
def rm_points(points, normals, rm_surface):
    ...
    """remove sample points based on their normal direction. Only consider cases for top and bottom points

    Args:
        points (ndarray): array of sample coordinates
        normals (ndarray): array of sample normals
        rm_surface (int): Specify the faces for removal: assign 1 for the top face, -1 for the bottom face, and 0 for both. The face with the lower z value represents the bottom face, while the one with the higher z value signifies the top face.

    Returns:
        ndarray: array of cleaned sample coordinates
        ndarray: array of cleaned sample normals
    """
    removed_points = []
    if rm_surface == 1:
        tob = 1
...
def reset_normals (samp_dist, shift_dist, tb_move, shapes_data, motl, bin_factor):
    ...
    tri_points, tri_normals = surfsamp.expand_points(tri_points, tri_normals, shift_dist, tb_move)  # negative for shifting in opposite direction
...
def get_sampling_pandas(shapes_data, overSample_dist, shift_dist = None, tb_dist = 0, rm_surface = None):
    ...
    if shift_dist !=  None:
        tri_points, tri_normals = surfsamp.expand_points(tri_points, tri_normals, shift_dist, tb_dist)  # negative for shifting in opposite direction, set 4 argument to 0 to omit moving of top and bottom layer.
    # remove unecessary points base on condition
    if rm_surface != None:
        tri_points, tri_normals = surfsamp.rm_points(tri_points, tri_normals, rm_surface)
...
def angles_clean(motl, vertices, normals, angle_threshold = 90, normal_vector = [0,0,1]):
    ...
    diff_angles = np.arccos(np.sum(motl_z_vector*surface_z_vector,axis=1))*180/np.pi
    ...
    clean_motl_list.df = motl.df[np.logical_or(diff_angles <= angle_threshold, top_score)]
