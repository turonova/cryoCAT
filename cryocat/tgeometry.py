...
def compute_barycentric_coord(triangle,point):
    ...
    if ((fabs(triangle_normal[0]) >= fabs(triangle_normal[1])) and (fabs(triangle_normal[0]) >= fabs(triangle_normal[2]))):
...
def normals_to_euler_angles(normals,convention='ZXZ'):
    ...
    psi  = 90 + np.degrees(np.arctan2(normals['y'].values,normals['x'].values))
    b_idx=np.where(np.arctan2(normals['y'].values,normals['x'].values)==0)
    psi[b_idx]=0
