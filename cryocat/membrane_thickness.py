...
1. Extract and refine the coordinates and orientations of surface points:
   - Instance segmentation inputs: membrane labels of interest can be specified using 
     a dictionary or in a config.yaml file
...
Outputs:
- Vertices CSV: coordinates (in voxels and in nm) and orientations of the sampled 
  surface points, including to which surface they've been assigned
- Thickness CSV: membrane thickness measurements (in nm, scaled by input voxel size),
  including the IDs and positions of paired points from each surface
- Statistics log: general statistics including mean/median thicknesses, interquartile 
  range, histograms, number of points, and measurement coverage
- (Optional) MRC files: coordinates of sampled surface points (voxel value = 1) to 
  overlay with original membrane segmentations
- (Optional) XYZ point clouds: coordinates of sampled surface points for visualization 
  in external software (e.g., MeshLab)
- (Optional) Thickness volumes: MRC files where voxel values correspond to measured 
  membrane thicknesses (in nm) for each surface point
...
class MeshNormalVoter:
    ...
    def refine_normals(self):
        ...
        temp_normals = np.zeros_like(self.refined_normals if self.refined_normals is not None 
                                   else self.initial_normals)
        ...
        for start_idx in tqdm(range(0, len(self.points), self.batch_size)):
            ...
            for i, point_neighbors in enumerate(neighbors):
                ...
                if len(point_neighbors) == 0:
                    temp_normals[point_idx] = (self.refined_normals if self.refined_normals is not None 
                                             else self.initial_normals)[point_idx]
                    continue
                
                # Compute weights
                distances = np.linalg.norm(
                    self.points[point_neighbors] - self.points[point_idx], axis=1)
                weights = np.exp(-distances**2 / (2 * self.sigma**2))
                
                # Weighted average of neighbor normals
                current_normals = (self.refined_normals if self.refined_normals is not None 
                                 else self.initial_normals)
                avg_normal = np.average(
                    current_normals[point_neighbors], 
                    weights=weights, 
                    axis=0
                )
        ...
        while queue:
            ...
            for neighbor in neighbors:
                if not oriented[neighbor]:
                    ...
                    dot_product = np.dot(self.refined_normals[current], 
                                       self.refined_normals[neighbor])
                    if dot_product < 0:
...
def is_surface_point(point, segmentation):
    ...
    for nx, ny, nz in neighbors:
        if (0 <= nx < segmentation.shape[0] and
            0 <= ny < segmentation.shape[1] and
            0 <= nz < segmentation.shape[2] and
            not segmentation[nx, ny, nz]):
            return True
...
def get_neighbor_surface_points(vertex, segmentation, include_edges=True):
    ...
    for new_x, new_y, new_z in neighbors_to_check:
        ...
        if (new_x < 0 or new_y < 0 or new_z < 0 or
            new_x >= segmentation.shape[0] or
            new_y >= segmentation.shape[1] or
            new_z >= segmentation.shape[2]):
            continue
...
def interpolate_surface_points(vertices, normals, segmentation, 
                          interpolation_points=1, include_edges=True, logger=None):
    """
    ...
    for i, vertex in enumerate(vertices):
        ...
        if (vertex_tuple not in processed_positions and 
            is_surface_point(vertex, segmentation)):
            ...
            neighbor_points = get_neighbor_surface_points(
                vertex, segmentation, include_edges=include_edges)
        ...
        if i < len(vertices) - 1:
            ...
            interp_points = interpolate_between_vertices(
                vertex, next_vertex, segmentation, interpolation_points)
...
def refine_mesh_normals(vertices, initial_normals, radius_hit=10.0, batch_size=2000, 
                       flip_normals=True, logger=None):
    """
...
def verify_and_save_outputs(aligned_vertices, aligned_normals, vertex_volume, 
                         surface1_mask, surface2_mask,
                         membrane_name, base_name, output_dir, voxel_size, origin,
                         save_vertices_mrc=False, save_vertices_xyz=False, logger=None):
    """
    ...
    try:
        ...
        if save_vertices_xyz:
            ...
            if line_count != len(aligned_vertices):
                log_msg(f"WARNING: XYZ file line count ({line_count}) doesn't match vertex count ({len(aligned_vertices)})")
...
@cuda.jit
def find_all_possible_matches_kernel(points, normals, surface1_mask, surface2_mask,
                                   match_distances, match_indices, match_counts,
                                   max_thickness_voxels, max_angle_cos, 
...
def process_matches_gpu2cpu(match_distances, match_indices, match_counts, n_points, 
                       max_matches_per_point, voxel_size):
    """
    ...
    for dist, surf1_idx, surf2_idx in all_matches:
        if (surf1_idx not in surface1_assigned and 
            surf2_idx not in surface2_assigned):
...
def measure_thickness_gpu(points, normals, surface1_mask, surface2_mask,
                       voxel_size, max_thickness_nm=8.0, max_angle_degrees=3.0,
                       direction="1to2", logger=None):
    """
    ...
    match_distances = cuda.to_device(
        np.zeros(n_points * max_matches_per_point, dtype=np.float32))
    match_indices = cuda.to_device(
        np.zeros(n_points * max_matches_per_point, dtype=np.int32))
    ...
    if np.sum(valid_mask) > 0:
        ...
        log_msg(f"Min: {np.min(thickness_results[valid_mask]):.2f} nm, Max: {np.max(thickness_results[valid_mask]):.2f} nm")
    
    return thickness_results, valid_mask, point_pairs

def generate_matching_statistics(thickness_results, valid_mask, point_pairs, 
                               points, surface1_mask, surface2_mask, voxel_size):
    """
...
def save_matching_statistics(stats, output_path, logger=None):
    ...
    with open(output_path, 'w') as f:
        ...
        if 'mean_thickness' in stats:
            ...
            for count, (left, right) in zip(hist['counts'], 
                                          zip(hist['bin_edges'][:-1], 
                                              hist['bin_edges'][1:])):
...
def generate_thickness_volume(points, thickness_results, valid_mask, segmentation, voxel_size, point_pairs):
    ...
    for point, pair_idx, thickness in zip(valid_points, valid_pairs, valid_thicknesses):
        # First surface point
        x1, y1, z1 = point
        if (0 <= x1 < thickness_volume.shape[0] and
            0 <= y1 < thickness_volume.shape[1] and
            0 <= z1 < thickness_volume.shape[2]):
            thickness_volume[x1, y1, z1] = thickness
        
        # Second surface point
        x2, y2, z2 = points[pair_idx].astype(int)
        if (0 <= x2 < thickness_volume.shape[0] and
            0 <= y2 < thickness_volume.shape[1] and
            0 <= z2 < thickness_volume.shape[2]):
...
def find_matches_parallel(points, normals, source_mask, target_mask, 
                          target_indices, max_thickness_voxels, 
                          max_angle_cos, match_distances, match_indices, match_counts):
    """
...
def measure_thickness_cpu(points, normals, surface1_mask, surface2_mask,
                         voxel_size, max_thickness_nm=8.0, max_angle_degrees=5.0,
                         direction="1to2", num_threads=None, logger=None,
                         max_matches_per_point=25):
    ...
    neighbor_lists = target_tree.query_ball_point(
        source_points, max_thickness_voxels)
    ...
    thickness_results, valid_mask, point_pairs = process_matches_cpu2cpu(
        flat_matches, n_points, voxel_size)
...
def process_matches_cpu2cpu(flat_matches, n_points, voxel_size):
    ...
    for dist, source_idx, target_idx in flat_matches:
        if (source_idx not in source_assigned and 
            target_idx not in target_assigned):
            # Assign match (still in voxel units)
...
def process_membrane_segmentation(segmentation_path, output_dir=None, config_path=None,
                                membrane_labels=None, mesh_sampling = 1,
                                interpolate=True, interpolation_points=1,
                                refine_normals=True, radius_hit=10.0, 
                                flip_normals=True, batch_size=2000,
                                save_vertices_mrc=False, save_vertices_xyz=False,
                                logger=None):
    """
    ...
    # Read segmentation
    segmentation, voxel_size, origin = read_segmentation(
...
def measure_membrane_thickness(segmentation_path, input_csv, output_csv=None, output_dir=None,
                             max_thickness=8.0, max_angle=3.0,
                             save_thickness_mrc=False, direction="1to2", 
                             use_gpu=True, num_cpu_threads=None, logger=None):
    """
    ...
    if use_gpu and gpu_available:
        ...
        thickness_results, valid_mask, point_pairs = measure_thickness_gpu(
            points, normals, surface1_mask, surface2_mask,
    ...
    stats = generate_matching_statistics(
        thickness_results, valid_mask, point_pairs,
        points, surface1_mask, surface2_mask, voxel_size
    )
...
def run_full_pipeline(segmentation_path, output_dir=None, config_path=None,
                     membrane_labels=None,
                     interpolate=True, interpolation_points=1,
                     refine_normals=True, radius_hit=10.0, flip_normals=True,
                     max_thickness=8.0, max_angle=3.0,
                     save_vertices_mrc=False, save_vertices_xyz=False, save_thickness_mrc=False,
                     direction="1to2", batch_size=2000,
                     use_gpu=True, num_cpu_threads=None):
    """
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Membrane Thickness Analysis Tool',
    ...
    parser.add_argument('--membrane_labels', type=str,
                  help='Comma-separated list of membrane labels in format name:value (e.g., "membrane:1,vesicle:2")')
    parser.add_argument('--interpolate', action='store_true',
                  help='Interpolate points for more uniform coverage')
    parser.add_argument('--interpolation_points', type=int, default=1,
                  help='Number of points to interpolate between adjacent vertices')
    parser.add_argument('--refine_normals', action='store_true',
                  help='Refine normals after running marching cubes')
    parser.add_argument('--flip_normals', action='store_true',
                  help='Flip normals inward after refinement')
    parser.add_argument('--radius_hit', type=float, default=10.0,
                  help='Search radius for normal refinement (in voxels)')
    
    # Thickness measurement options
    parser.add_argument('--max_thickness', type=float, default=8.0,
                  help='Maximum thickness in nm (will be converted to voxels internally)')
    parser.add_argument('--max_angle', type=float, default=3.0,
                  help='Maximum angle for cone search')
    parser.add_argument('--direction', choices=['1to2', '2to1'], default='1to2',
                  help='Direction of thickness measurement: surface 1 to 2 or surface 2 to 1')
    
    # Output options
    parser.add_argument('--save_vertices_mrc', action='store_true',
                  help='Save vertices posiitons in MRC format, e.g., for overlaying with the original segmentation')
    parser.add_argument('--save_vertices_xyz', action='store_true',
                  help='Whether to save vertex coordinates as point cloud XYZ files, e.g., for visualizing in MeshLab')
    parser.add_argument('--save_thickness_mrc', action='store_true',
    ...
    hardware_group.add_argument('--use_cpu', action='store_true',
                       help='Force CPU implementation even if GPU is available')
    hardware_group.add_argument('--cpu_threads', type=int,
    ...
    # Pipeline control
    parser.add_argument('--mode', choices=['full', 'surface', 'thickness'], default='full',
                  help='Processing mode: full pipeline, surface extraction only, or thickness measurement only')
    parser.add_argument('--input_csv', 
                  help='Input CSV for thickness measurement mode (only used with --mode=thickness)')
