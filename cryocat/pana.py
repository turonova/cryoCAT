...
def generate_wedgemask_slices_tile(wedgelist, tile_filter):
    ...
    wedge_slices = np.fft.ifftshift(
        np.transpose(np.tile(tile_bin_slice, (tile_size[1], 1, 1)), (2, 0, 1))                                   
    ) 
...
def generate_wedge_masks(
    ...
    if exposure_weighting:
        filter_template *= generate_exposure(wedgelist, 
                                             active_slices_idx, 
                                             wedge_slices_weights, 
                                             binning)

    if ctf_weighting:
        filter_template *= generate_ctf(wedgelist, 
                                        active_slices_idx, 
                                        wedge_slices_weights, 
                                        binning)
...
def create_subtomo_name(structure_name, motl_name, tomo_id, boxsize):
    ...
    subtomo_name = "subtomo_" + structure_name + "_m" + motl_name \
                   + "_t" + tomo_id + "_s" + str(boxsize) + ".em"
...
def create_wedge_names(wedge_path, tomo_number, boxsize, binning, filter=None):
    ...
    file_ending = str(boxsize) + "_t" + str(tomo_number) + "_b" + str(binning) \
                  + "_f" + str(filter) + ".em"
...
def create_output_folder_path(folder_path, structure_name, folder_spec):
    ...
    if isinstance(folder_spec, int):
        output_path = (
            ...
            + create_output_folder_name(folder_spec)
            + "/"
        )
...
def create_subtomograms_for_tm(template_list, parent_folder_path):
    ...
    for i, entry in enumerate(unique_entries):
        if np.all(temp_df.loc[entry_indices[i], "Tomo created"]):
            ...
            temp_df.loc[create_idx, ["Phi", "Theta", "Psi"]] = np.tile(
                subtomo_rotation, (create_idx.shape[0], 1))
...
def get_mask_stats(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        soft_mask = cryomap.read(create_em_path(parent_folder_path, 
                                                structure_name, 
                                                temp_df.at[i, "Mask"]))
        sharp_mask = cryomap.read(create_em_path(parent_folder_path, 
                                                 structure_name, 
                                                 temp_df.at[i, "Tight mask"]))
...
def compute_sharp_mask_overlap(
    ...
    for i in indices:
        ...
        structure_name = temp_df.at[i, "Structure"]
        mask_name = create_em_path(parent_folder_path, 
                                   structure_name, 
                                   temp_df.at[i, "Tight mask"])
        ...
        output_folder = create_output_folder_path(parent_folder_path, 
                                                  structure_name, 
...
def check_existing_tight_mask_values(
    ...
    for i in indices:
        ...
        if "Tight mask overlap" not in rot_info.columns:
            tm_spec = {"Done": True, 
                       "Tight mask": temp_df.at[i, "Tight mask"], 
            ...
            for j in done_idx:
                csv_file = (
                    create_output_folder_path(
                        parent_folder_path, 
                        temp_df.at[j, "Structure"], 
                        temp_df.at[j, "Output folder"]
                    )
...
def compute_dist_maps_voxels(
    ...
    - Labeled masks and morphologically opened masks are saved as `.em` files with
      `_label` and `_label_open` suffixes, respectively.
    """
    ...
    for i in indices:
        ...
        for j, value in enumerate(dist_names):
            ...
            cryomap.write(
                dist_label, 
                output_folder + output_base + "_angles_" + value + "_label.em", 
                data_type=np.single
            )

            # remove labels that are smaller than the footprint 
            open_label = morphology.binary_opening(dist_label, 
                                                   footprint=np.ones(morph_footprint), 
                                                   out=None)
...
def compute_center_peak_stats_and_profiles(template_list, indices, parent_folder_path):
    ...
    7. Updates the template list CSV file with the computed statistics:
        * "Peak value"                                                                  
    ...
    for i in indices:
        ...
        for r in range(1, 6):
            cc_mask = cryomask.spherical_mask(np.asarray(scores_map.shape), 
                                              radius=r, 
                                              center=peak_center)
...
def analyze_rotations(
    ...
    cc_mask = cryomask.spherical_mask(np.array(output_size), 
                                      radius=cc_radius).astype(np.single)
    ...
    ang_dist, cone, inplane = geom.compare_rotations(starting_angles, 
                                                     angles, 
    ...
    for i, a in enumerate(angles):
        ...
        cc_map = cryomap.calculate_flcf(masked_ref, 
                                        rot_mask, 
                                        conj_target=conj_target, 
                                        conj_target_sq=conj_target_sq)
    ...
    if output_file is not None:
        ...
        cryomap.write(file_name=output_file + "_scores.em", 
                      data_to_write=final_ccc_map, 
                      data_type=np.single)
        # cryomap.write(file_name=output_file + '_scores_masked.em', 
        #               data_to_write = final_ccc_map_masked, 
        #               data_type=np.single)
        cryomap.write(file_name=output_file + "_angles.em", 
                      data_to_write=final_angles_map, 
                      data_type=np.single)

    return res_table, final_ccc_map, final_angles_map, final_ccc_map_masked


def run_analysis(template_list, 
                 indices, 
                 angle_list_path, 
                 wedge_path, 
                 parent_folder_path, 
    ...
    template_list : str or path-like
        ...
        - "Mask"
        - "Angles"
        - "Compare" (compare method; "tmpl": tmpl vs. tmpl, "subtomo": subtomo vs. 
          sutomo, or else)
    ...
    for i in indices:
        ...
        if temp_df.at[i, "Compare"] == "tmpl":
            ...
            tomo = create_em_path(parent_folder_path, 
                                  structure_name, 
                                  temp_df.at[i, "Tomo map"])
            ...
            if temp_df.at[i, "Apply wedge"]:
                wedge_tomo, wedge_tmpl = create_wedge_names(
                    wedge_path, 
                    tomo_number, 
                    temp_df.at[i, "Boxsize"], 
                    temp_df.at[i, "Binning"]
                )
        else:
            tomo = create_em_path(parent_folder_path, 
                                  temp_df.at[i, "Compare"], 
                                  temp_df.at[i, "Tomo map"])
        ...
        _, _, _ = tmana.create_angular_distance_maps(angles_map, 
                                                     angle_list, 
                                                     write_out_maps=True)
...
def run_angle_analysis(
    ...
    for i in indices:
        ...
        if write_output:
            ...
            hist_df = pd.DataFrame(data=final_hist, 
...
def create_summary_pdf(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        figure_title = (
            structure_name 
            + " (" 
            + temp_df.at[i, "Template"] 
            + ") matched with " 
            + title_end
        )

        extenstion_list = ["_scores.em", 
                           "_angles_dist_all.em", 
                           "_angles_dist_normals.em", 
                           "_angles_dist_inplane.em"]
        ...
        cross_slices = []
        tight_mask = cryomap.read(create_em_path(parent_folder_path, 
                                                 structure_name, 
                                                 temp_df.at[i, "Tight mask"]))
        ...
        for m, ext_name in enumerate(extenstion_list):
            ...
            if m == 0:
                cross_slices.append(cryomap.get_cross_slices(input_map, 
                                                             slice_numbers=peak_center, 
                                                             axis=[0, 1, 2]))

            cross_slices.append(
                cryomap.get_cross_slices(input_map, 
                                         slice_half_dim=5, 
                                         slice_numbers=peak_center, 
                                         axis=[0, 1, 2])
            )
        ...
        rot_info = pd.read_csv(output_folder + output_base + ".csv", index_col=0)
        line_profiles = pd.read_csv(output_folder + output_base + "_peak_line_profiles.csv", 
                                    index_col=0)
        ...
        ad_plt = sns.scatterplot(
            ax=fig.add_subplot(grid_rows[0][1, 1]), data=rot_info, x="ang_dist", 
                               y="ccc_masked", linewidth=0, s=5)
        ad_plt.set(ylabel=None, xlabel="Angular distance (in degrees)")
        ad_plt = sns.lineplot(ax=fig.add_subplot(grid_rows[0][1, 2]), 
                              data=line_profiles[["x", "y", "z"]])
        ...
        if add_hist:
            ...
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 1.0, num=100), 
                         y="ang_dist")
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 1.0, num=100), 
                         y="cone_dist")
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 1.0, num=100), 
                         y="inplane_dist")
            hist_plt.set(ylim=(0, 250), ylabel="Number of CCC values (bin size 0.1)", 
                         xlabel="CCC")

            hist_info = pd.read_csv(output_folder + output_base + "_gradual_angles_analysis.csv", 
                                    index_col=0)
            hist_plt = fig.add_subplot(grid_rows[1][0, 1])
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 359.0, num=359), 
                         y="ccc_masked")
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 359.0, num=359), 
                         y="cone_ccc_masked")
            sns.lineplot(ax=hist_plt, data=hist_info, x=np.linspace(0.0, 359.0, num=359), 
                         y="inplane_ccc_masked")
        ...
        plt.savefig(output_folder + output_base + "_summary.pdf", 
                    transparent=True, 
                    bbox_inches="tight")
        plt.close()
...
def get_shape_stats(template_list, indices, shape_type, parent_folder_path):
    ...
    for i in indices:
        structure_name = temp_df.at[i, "Structure"]
        sharp_mask = cryomap.read(create_em_path(parent_folder_path, structure_name, 
                                                 temp_df.at[i, "Tight mask"]))
...
def plot_scores_and_peaks(peak_files, plot_title=None, output_file=None):
    ...
    row_size = 4 * n_rows
    fig, axs = plt.subplots(n_rows, 4, figsize=(row_size, 100), 
...
def compute_peak_shapes(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        output_base = create_output_base_name(i)
        output_folder = create_ouptut_folder_path(parent_folder_path, structure_name, 
                                                  temp_df.at[i, "Output folder"])
        scores_map = cryomap.read(output_folder + output_base + "_scores.em")
        ...
        plot_scores_and_peaks(
            [scores_map, t_th_map, t_surf, t_map, g_th_map, 
             g_surf, g_map, h_th_map, h_surf, h_map],
...
def rename_folders(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        structure_path = create_structure_path(parent_folder_path, 
                                               temp_df.at[i, "Structure"])
...
def rename_scores_angles(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        if temp_df.at[i, "Compare"] == "tmpl":
            ...
            tomo = create_em_path(parent_folder_path, structure_name, 
                                  temp_df.at[i, "Tomo map"])
            tomo_number = re.findall(r"\d+", temp_df.at[i, "Tomogram"])[0]
            comp_type = "ts_t" + tomo_number + "_"
        else:
            tomo = create_em_path(parent_folder_path, temp_df.at[i, "Compare"], 
                                  temp_df.at[i, "Tomo map"])
            comp_type = "td_" + temp_df.at[i, "Compare"] + "_"

        output_base = comp_type + temp_df.at[i, "Map type"]
        new_base = create_output_base_name(i)
        output_folder = create_output_folder_path(parent_folder_path, structure_name, 
                                                  temp_df.at[i, "Output folder"])
...
def recompute_dist_maps(template_list, indices, parent_folder_path, angle_list_path):
    ...
    for i in indices:
        ...
        angle_list = angle_list_path + temp_df.at[i, "Angles"]
        c_symmetry = temp_df.at[i, "Symmetry"]
        _, _, _ = tmana.create_angular_distance_maps(angles_map, angle_list, 
                                                     write_out_maps=True, 
                                                     c_symmetry=c_symmetry)
