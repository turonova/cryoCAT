...
def create_tomo_name(folder_path, tomo, ):
...
def cut_the_best_subtomo(tomogram,motl_path,subtomo_shape,output_file):
    ...
    subvolume = cryomaps.extract_subvolume(tomo, coord, subtomo_shape)
    subvolume_sh = cryomaps.shift2(subvolume,shifts)
    #subvolume_rot = cryomaps.rotate(subvolume_sh,rotation_angles=angles)
    
    if output_file is not None:
        cryomaps.write(subvolume_sh,output_file,data_type=np.single)
...
def create_subtomograms_for_tm(template_list, parent_folder_path):
    ...
    for i, entry in enumerate(unique_entries):
        if np.all(temp_df.loc[entry_indices[i], 'Tomo created']):
            ...
            _, subtomo_rotation = cut_the_best_subtomo(create_tomo_name(parent_folder_path,entry[2]),
                                                       motl,
...
def get_mask_stats(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        soft_mask = cryomaps.read(create_em_path(parent_folder_path,structure_name,temp_df.at[i, 'Mask']))
        sharp_mask = cryomaps.read(create_em_path(parent_folder_path,structure_name,temp_df.at[i, 'Tight mask']))
...
def compute_sharp_mask_overlap(template_list, indices, angle_list_path, parent_folder_path):
    ...
    for i in indices:
        ...
        for j in rotations:
            mask_rot = cryomaps.rotate(mask,rotation=j,transpose_rotation=True)
...
def check_existing_tight_mask_values(template_list, indices, parent_folder_path, angle_list_path):
    ...
    for i in indices:
        ...
        if 'Tight mask overlap' not in rot_info.columns:
            ...
            for j in done_idx:
                csv_file = create_output_folder_path(parent_folder_path, temp_df.at[j, 'Structure'], temp_df.at[j, 'Output folder']) + create_output_base_name(j) + '.csv'
...
def compute_dist_maps_voxels(template_list, indices, parent_folder_path, morph_footprint=(2,2,2)):
    ...
    for i in indices:
        ...
        output_base = create_output_base_name(i)
        output_folder = create_output_folder_path(parent_folder_path, structure_name, temp_df.at[i, 'Output folder'])
        scores_map = cryomaps.read(output_folder + output_base + '_scores.em')
        ...
        for j, value in enumerate(dist_names):
            dist_map = cryomaps.read(output_folder + output_base + '_angles_' + value + '.em')
            ...
            cryomaps.write(dist_label,output_folder + output_base + '_angles_' + value + '_label.em',data_type=np.single)
            ...
            temp_df.at[i, column_name ] = label_vc
            cryomaps.write(open_label,output_folder + output_base + '_angles_' + value + '_label_open.em',data_type=np.single)
...
def compute_center_peak_stats_and_profiles(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        for j, dim in enumerate(['x','y','z']):
            peak_difference = peak_value - (line_profiles[peak_center[j]-1,j] + line_profiles[peak_center[j]+1,j]) / 2.0
...
def analyze_rotations(tomogram, template, template_mask, input_angles, wedge_mask_tomo = None, wedge_mask_tmpl = None, output_file=None, cc_radius=3, angular_offset = None, starting_angle = np.asarray([0,0,0]), c_symmetry = 1):
    ...
    if wedge_mask_tomo is not None:
        wedge_tomo = cryomaps.read(wedge_mask_tomo)
        conj_target,conj_target_sq=cryomaps.calculate_conjugates(tomo, wedge_tomo)
    else:
        conj_target,conj_target_sq=cryomaps.calculate_conjugates(tomo)

    if wedge_mask_tmpl is not None:
        wedge_tmpl = cryomaps.read(wedge_mask_tmpl)
    ...
    res_table = pd.DataFrame(columns=['ang_dist', 'cone_dist', 'inplane_dist', 'common_voxels', 'ccc', 'ccc_masked'], dtype=float)
    ...
    for i, a in enumerate(angles):
        ...
        rot_ref=cryomaps.rotate(tmpl,rotation_angles=a,spline_order=1).astype(np.single)
        rot_mask=cryomaps.rotate(mask,rotation_angles=a,spline_order=1).astype(np.single)
        ...
        norm_ref=cryomaps.normalize_under_mask(rot_ref,rot_mask)
        masked_ref=norm_ref*rot_mask

        cc_map=cryomaps.calculate_flcf(masked_ref,rot_mask,conj_target=conj_target,conj_target_sq=conj_target_sq)
        ...
        res_table.loc[len(res_table)] = [ 
            ...
            np.clip(np.max(cc_map),0.0, 1.0),
            np.clip(np.max(masked_map),0.0, 1.0)
        ]
    ...
    if output_file is not None:
        ...
        cryomaps.write(file_name=output_file + '_scores.em', data_to_write = final_ccc_map, data_type=np.single)
        #cryomaps.write(file_name=output_file + '_scores_masked.em', data_to_write = final_ccc_map_masked, data_type=np.single)
        cryomaps.write(file_name=output_file + '_angles.em', data_to_write = final_angles_map, data_type=np.single)
    ...
    for i in indices:
        ...
        if temp_df.at[i, 'Compare'] == 'tmpl':
            ...
            if temp_df.at[i, 'Apply wedge']:
                wedge_tomo, wedge_tmpl = create_wedge_names(wedge_path, tomo_number,  temp_df.at[i, 'Boxsize'],  temp_df.at[i, 'Binning'])
        else:
...
def run_angle_analysis(template_list, indices, wedge_path, parent_folder_path, angular_range = 359, write_output = False, cc_radius_tol = 10):
    ...
    for i in indices:
        ...
        results = np.zeros((angular_range,6,3))
        ...
        ang_dist = pd.DataFrame(data=results[:,:,0], columns=['ang_dist', 'cone_dist', 'inplane_dist', 'common_voxels', 'ccc', 'ccc_masked'])
        ang_cone = pd.DataFrame(data=results[:,:,1], columns=['cone_ang_dist', 'cone_cone_dist', 'cone_inplane_dist', 'cone_common_voxels', 'cone_ccc', 'cone_ccc_masked'])
        ang_inplane = pd.DataFrame(data=results[:,:,2], columns=['inplane_ang_dist', 'inplane_cone_dist', 'inplane_inplane_dist', 'inplane_common_voxels', 'inplane_ccc', 'inplane_ccc_masked'])
        final_df = pd.concat([ang_dist, ang_cone, ang_inplane], axis=1)

        if write_output:
            output_base = create_output_folder_path(parent_folder_path, structure_name, temp_df.at[i, 'Output folder']) + create_output_base_name(i)
...
def create_summary_pdf(template_list, indices, parent_folder_path):
    ...
    for i in indices:
        ...
        cross_slices = []
        tight_mask = cryomaps.read(create_em_path(parent_folder_path, structure_name,temp_df.at[i, 'Tight mask']))
        cross_slices.append(cryomaps.get_cross_slices(tight_mask))

        for m, ext_name in enumerate(extenstion_list):
            input_map = cryomaps.read(output_folder + output_base + ext_name)
            
            if m == 0:
                cross_slices.append(cryomaps.get_cross_slices(input_map, slice_numbers=peak_center, axis=[0, 1, 2]))

            cross_slices.append(cryomaps.get_cross_slices(input_map, slice_half_dim = 5, slice_numbers=peak_center, axis=[0, 1, 2]))
        ...
        grid_rows.append(gridspec.GridSpecFromSubplotSpec(6, 4, subplot_spec=grid_base[last_row], 
                                        height_ratios = [unit_size, unit_size, unit_size, unit_size, unit_size, unit_size], width_ratios=widths))
        ...
        ad_plt = sns.scatterplot(ax=fig.add_subplot(grid_rows[0][1, 0]), data=rot_info, x='Tight mask overlap', y ='ccc_masked', linewidth=0, s = 5)
        ad_plt.set(ylabel='CCC', xlabel = 'Tight mask overlap (in voxels)')
        ad_plt = sns.scatterplot(ax=fig.add_subplot(grid_rows[0][1, 1]), data=rot_info, x='ang_dist', y ='ccc_masked', linewidth=0, s = 5)
        ...
        for c, slice  in enumerate(cross_slices):
            ...
            sns.heatmap(ax=hm1, data=np.flipud(slice[0].T), annot=use_annot, fmt=f_fmt, square = True, cmap=c_scheme, yticklabels = False, xticklabels = False, vmin = 0, vmax = data_max, cbar_ax=cb  )
            sns.heatmap(ax=hm2, data=np.flipud(slice[1].T), annot=use_annot, fmt=f_fmt, square = True, cmap=c_scheme, yticklabels = False, xticklabels = False, vmin = 0, vmax = data_max, cbar=False )
            sns.heatmap(ax=hm3, data=np.flipud(slice[2].T), annot=use_annot, fmt=f_fmt, square = True, cmap=c_scheme, yticklabels = False, xticklabels = False, vmin = 0, vmax = data_max, cbar=False )
...
def get_shape_stats(template_list, indices, shape_type, parent_folder_path):
    ...
    for i in indices:
        ...
        mask_stats = pd.DataFrame(measure.regionprops_table(mask_label,
                    properties=('label', 
                                'area',
                                'area_bbox',
                                'area_convex',
        ...
        output_base = create_structure_path(parent_folder_path, structure_name) + temp_df.at[i, 'Output folder'] + '/id_' + str(i) + '_shape_stats_'
...
def plot_scores_and_peaks(peak_files, plot_title = None, output_file = None):
    ...
    for i, p in enumerate(peak_files):
        ...
        sns.heatmap(ax=axs[i][0], data=np.flipud(peaks[:,:,0].T), square = True, cmap="viridis", annot=False, yticklabels = False, xticklabels = False, vmin = data_min, vmax = peak_height, cbar_ax=axs[i][3]  )
        sns.heatmap(ax=axs[i][1], data=np.flipud(peaks[:,:,1].T), square = True, cmap="viridis", annot=False, yticklabels = False, xticklabels = False, vmin = data_min, vmax = peak_height, cbar=False )
        sns.heatmap(ax=axs[i][2], data=np.flipud(peaks[:,:,2].T), square = True, cmap="viridis", annot=False, yticklabels = False, xticklabels = False, vmin = data_min, vmax = peak_height, cbar=False )
...
def compute_peak_shapes(template_list, indices, parent_folder_path):
     ...
     for i in indices:
        ...
        t_map, tp_shape, peak_value, t_th_map, t_surf = tmana.evaluate_scores_map(scores_map, label_type = 'ellipsoid', threshold_type = 'triangle')
        g_map, gp_shape, _, g_th_map, g_surf = tmana.evaluate_scores_map(scores_map, label_type = 'ellipsoid', threshold_type = 'gauss')
        h_map, hp_shape, _, h_th_map, h_surf = tmana.evaluate_scores_map(scores_map, label_type = 'ellipsoid', threshold_type = 'hard')
        ...
        plot_scores_and_peaks([scores_map, t_th_map, t_surf,  t_map, g_th_map, g_surf, g_map, h_th_map, h_surf, h_map ], plot_title = structure_name + ' id' + str(i), output_file = output_folder + 'peaks.png')
